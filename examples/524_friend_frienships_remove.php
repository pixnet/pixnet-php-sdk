...
$query2 = $_POST['query2'];
if ('' != $query2) {
    $response = $pixapi->friend->friendships->remove($query, $query2);
}
?>
<!DOCTYPE html>
<html>
...
<div class="container">
    ...
    <h3>呼叫方式</h3>
    <pre>$pixapi->friend->friendships->remove($id, $name);</pre>
    <div class="panel panel-primary">
      ...
      <div class="panel-body">
        ...
        <?php if ('' != $query2) { ?>
        <h3>執行</h3>
        <pre>$pixapi->friend->friendships->remove('<?= htmlspecialchars($query) ?>','<?= htmlspecialchars($query2) ?>');</pre>
        <h3>執行結果</h3>
