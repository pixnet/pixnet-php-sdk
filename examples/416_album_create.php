...
<div class="container">
    ...
    <h3>呼叫方式</h3>
    <pre>$pixapi->album->sets->create($title, $desc, <a href="http://developer.pixnet.pro/#!/doc/pixnetApi/albumSetComments" target="blank">$options</a> = array());</pre>
    <div class="well">
        <p>必填參數</p>
        <ul>
        <li><p>title</p>
    ...
    <form class="form-horizontal" role="form" method="POST">
      <div class="form-group">
        ...
        <div class="col-sm-5">
            <input type="text" class="form-control" id="query" name="title" placeholder="請輸入標題" value="<?= $_POST['title']? $_POST['title'] : 'PIXNET_SDK' ?>">
        </div>
      </div>
      <div class="form-group">
        <label class="col-sm-2 control-label" for="query">描述</label>
        <div class="col-sm-5">
            <input type="text" class="form-control" id="query" name="desc" placeholder="請輸入描述" value="<?= $_POST['desc']? $_POST['desc'] : '這是 PIXNET_SDK 建立的相簿' ?>">
        </div>
      </div>
      <button type="submit" class="btn btn-primary">建立相簿</button>
    </form>
    <?php if (!empty($_POST['title']) and !empty($_POST['desc'])) {?>
    <h3>實際執行</h3>
    <pre>
        $pixapi->album->sets->create(<?= htmlspecialchars($_POST['title'])?>, <?= $_POST['desc'] ?>, $options)
    </pre>
    <h3>執行結果</h3>
    <pre><?php print_r($pixapi->album->sets->create($_POST['title'], $_POST['desc'])); ?></pre>
    <?php }?>
</div>
</body>
</html>
